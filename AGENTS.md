...
Binary patch mode uses original 0-based byte offsets with `patchMode: "binary"`. Instructions are `OFFSET-LEN:HEX` to replace `LEN` bytes at `OFFSET`, `***DELETE***OFFSET-LEN` to delete `LEN` bytes at `OFFSET`, and `***APPEND***OFFSET-LEN:HEX` to insert `LEN` decoded bytes at `OFFSET`; use offset `0` for the start and `-1` for EOF append. `LEN` must match the decoded HEX byte count. Results include `metadata.diff` as a unified diff over hexdump text for diff UIs. Example: `{ "tool": "FileAction", "params": { "mode": "patch", "filePath": "data.bin", "patchMode": "binary", "patchText": "10-2:AA BB", "hashCheckMode": true, "hashCode": "sha256:..." } }`.
...
`exbash` inputs are intentionally small: `command`, `filePath`, attach `text` (stdin content), and attach file contents are limited to 4096 bytes; `description` is limited to 100 bytes and `asyncID` is limited to 50 bytes. Oversized inputs are rejected.
