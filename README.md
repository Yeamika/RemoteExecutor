...
Patch mode uses `mode: "patch"`, `patchText`, optional `patchMode` (`text` by default, or `binary`), optional `hashCheckMode`, and optional `hashCode`. Text patch syntax uses current 1-based line numbers, with one instruction per line: `***DELETE*** start-end`, `***MOVE*** start-end,startline` to move a block after `startline`, `***APPEND_HEAD*** startline` followed by literal lines and `***APPEND_END***`, and `n:new line text` to replace line `n`. Use `***APPEND_HEAD*** 0` to insert at the start. Example: `{ "tool": "FileAction", "params": { "mode": "patch", "filePath": "src/foo.rs", "patchText": "11:new line\n***APPEND_HEAD*** 11\nextra\n***APPEND_END***", "hashCheckMode": true, "hashCode": "sha256:..." } }`.
